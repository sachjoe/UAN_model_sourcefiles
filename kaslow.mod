...
BREAKPOINT {
	...
	gka = gbar1*n*h1*frac1+ gbar2*n*h2*frac2
	ik = gka*(v - ek)
}
...
PROCEDURE rates(v(mV)) { 
		...
		h1tau = (A1 + B1*exp(-2*((v+xc1)/yc1)^2))*0.4 : Both h1tau and h2tau Data fit: Yoshimura et al., 2006
		h2tau = (A2 + B2*exp(-((v-xc2)/yc2)^2))*0.4
}
